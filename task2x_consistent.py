...
class MultiTaskLongformerModel(nn.Module):
    def __init__(self):
        ...
        # Decade head - just predicts the decade (0-9)
        self.decade_classifier = nn.Sequential(
            nn.Linear(self.longformer.config.hidden_size, 512),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(512, 10)  # 10 classes for all possible decade values (0-9)
        )
    ...
    def predict_consistent(self, input_ids, attention_mask):
        """Get predictions ensuring century and decade are compatible"""
        century_logits, decade_logits = self.forward(input_ids, attention_mask)
        
        # Get probabilities
        century_probs = torch.softmax(century_logits, dim=1)
        decade_probs = torch.softmax(decade_logits, dim=1)
        
        # Get independent predictions
        century_pred = torch.argmax(century_probs, dim=1)
        decade_pred = torch.argmax(decade_probs, dim=1)
        
        # Calculate combined prediction value
        combined_pred = century_pred * 10 + decade_pred
        
        # Check if combined value is valid (under 43)
        invalid_mask = combined_pred >= 43
        if torch.any(invalid_mask):
            # For invalid combinations, adjust the century down to make it valid
            century_pred[invalid_mask] = 4  # Force to century 4 (max valid)
            combined_pred[invalid_mask] = century_pred[invalid_mask] * 10 + decade_pred[invalid_mask]
        
        return century_pred, decade_pred, combined_pred


def multi_task_loss(century_logits, decade_logits, 
                    century_labels, decade_labels, 
                    century_weight=0.5, decade_weight=0.5):
    # Standard cross-entropy loss for century classification
    century_loss = nn.CrossEntropyLoss()(century_logits, century_labels)
    
    # Standard cross-entropy loss for decade classification (0-9)
...
def evaluate(model, dataloader):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            century_predictions, decade_predictions, combined_predictions = model.predict_consistent(
                input_ids=batch['input_ids'],
                attention_mask=batch['attention_mask']
            )
            ...
            combined_correct += (combined_predictions == combined_labels).sum().item()
            ...
            combined_pred_values.extend(combined_predictions.cpu().numpy())
    ...
    combined_accuracy = combined_correct / total_predictions
    ...
    return average_loss, century_accuracy, century_mae, decade_accuracy, decade_mae, combined_accuracy, final_rank
...
def train_multi_task_model(model, train_loader, val_loader, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        val_loss, val_century_accuracy, val_century_mae, val_decade_accuracy, val_decade_mae, val_combined_accuracy, val_final_rank = evaluate(model, val_loader)
        ...
        if val_loss < best_val_loss:
            ...
            model_path = os.path.join(model_save_dir, f'best_model_epoch_{epoch+1}_classification.pt')
        ...
        print(f"Epoch {epoch+1}: Century Accuracy = {val_century_accuracy:.4f}, Century MAE = {val_century_mae:.4f}, " 
              ...
              f"Combined Accuracy = {val_combined_accuracy:.4f}, Final Rank = {val_final_rank:.4f}, Val Loss = {val_loss:.4f}")
    
    # Load the best model
    if best_model_state is not None:
        model.load_state_dict(best_model_state)
        # Save the final best model
        final_model_path = os.path.join(model_save_dir, 'best_model_classification.pt')
