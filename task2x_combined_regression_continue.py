...
def train_single_head_model(model, train_loader, val_loader, epochs=10, resume_from=None, start_epoch=0):
    ...
    if resume_from and os.path.exists(resume_from):
        print(f"Loading model state from {resume_from}")
        checkpoint = torch.load(resume_from)
        model.load_state_dict(checkpoint)
        best_val_loss = 2.9250  # As provided in the prompt
...
if __name__ == "__main__":
    ...
    # Path to the checkpoint from epoch 1
    checkpoint_path = 'models/task2x/best_single_head_regression_model_epoch_1.pt'
    
    print("Resuming training from epoch 2...")
    trained_model = train_single_head_model(
        model, 
        train2x_dataloader, 
        valid2x_dataloader, 
        epochs=10, 
        resume_from=checkpoint_path, 
        start_epoch=1  # Start from epoch 1 (which is the 2nd epoch since we count from 0)
    )
